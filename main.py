...
# 
# # 3. '설정' 메뉴 안에 '카테고리 관리' 항목 추가
# setting_menu.add_command(label="⚙️ 카테고리 관리", command=lambda: CategoryWindow(window))
# 
# # 4. 메뉴바에 '설정' 메뉴 등록
...
# 1. 최상단 프레임 (높이 36px, 깔끔한 연회색 배경)
top_bar = tk.Frame(window, bg="#f1f5f9", height=30)
top_bar.pack(side="top", fill="x")
top_bar.pack_propagate(False)  # 지정한 높이 고정

# 2. 드롭다운 팝업 메뉴 설정 (디자인 커스텀)
setting_popup = tk.Menu(
    window,
    tearoff=0, # tearoff: 메뉴 맨 위에 점선 (tearoff=1: 점선 O / tearoff=0: 점선 X)
    bg="#ffffff",
    fg="#1e293b",
    activebackground="#e2e8f0", # 마우스 올렸을 때 배경색
    activeforeground="#1f497d", # 마우스 올렸을 때 글자색
    font=("맑은 고딕", 10),
    bd=1,
    relief="solid"
)
...
# 메뉴 항목 추가
setting_popup.add_command(label="카테고리 관리", command=lambda: open_category_window()) # add_command(): 클릭 가능한 항목을 추가하는 함수. menu.add_command(label="메뉴 이름", command=실행할 함수)

# 3. 상단바에 들어갈 '설정' 드롭다운 버튼
btn_setting = tk.Menubutton(
    top_bar,
    text="⚙️ 설정",
    font=("맑은 고딕", 9, "bold"),
    bg="#f1f5f9",
...
btn_setting.pack(side="left", padx=12, fill="y")

# 버튼에 팝업 메뉴 연결
btn_setting.config(menu=setting_popup) # config(): 위젯의 설정값을 변경하는 함수

# 4. 클릭 시 메뉴를 버튼 바로 밑에 정확히 띄우는 함수
def show_menu(event):
    # 버튼의 화면상 절대 좌표 계산
    x = btn_setting.winfo_rootx() # winfo_rootx():btn_setting의 모니터 화면 전체 기준 X 좌표(가로 위치) 가져옴
    y = btn_setting.winfo_rooty() + btn_setting.winfo_height() # winfo_rooty(): 버튼의 모니터 화면 기준 Y 좌표(세로 위쪽 위치) 가져옴 / winfo_height(): 버튼 높이
    # 모니터 전체 화면에서 위치를 가져오는 이유: post() 함수가 전체 화면 좌표를 기반으로 메뉴를 띄우기 때문
    setting_popup.post(x, y) # post(x, y): 생성해둔 팝업 메뉴(setting_popup)을 전달받은 화면 좌표 위치에 출력하라는 함수

# 버튼 클릭 이벤트 바인딩 (좌클릭 시 show_menu 실행)
btn_setting.bind("<Button-1>", show_menu)
